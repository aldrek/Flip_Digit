...
import static com.a.flipdigit.R.styleable.FlipItem_tintColor;
...
public class FlipItem extends RelativeLayout {
    ...
    public FlipItem(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.FlipItem);
        final int num = typedArray.getIndexCount();

        try {

            isFastFlip = typedArray.getBoolean(FlipItem_isFastFlip, false);

            // Set tint
            for (int i = 0; i < num; i++) {
                int attr = typedArray.getIndex(i);

                if (attr == FlipItem_tintColor)
